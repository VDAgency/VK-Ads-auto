...
class SessionStore:
    ...
    def save(self, sender_id: int, session_str: str) -> None:
        ...
        Пустая строка отвергается: `StringSession.save()` возвращает `""`, пока ключ
        авторизации обнулён — ровно так выглядит сессия в середине переезда между
        дата-центрами. Записать такое значит затереть рабочую сессию оператора.
        """
        if not session_str:
            raise ValueError("пустая строка сессии — сохранение затёрло бы рабочую сессию")
