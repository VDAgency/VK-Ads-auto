...
class UserbotSettings(BaseSettings):
    ...
    # Как часто фоновая проверка обновляет состояние сессий. Сессия Telegram сама не
    # протухает, поэтому чаще смысла нет; реже — поломка обнаружится слишком поздно.
    keepalive_interval: float = 300.0
    # Сколько живёт незавершённая авторизация (оператор начал привязку и бросил).
    pending_auth_ttl: float = 300.0
