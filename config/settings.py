...
class Settings(BaseSettings):
    ...
    # Мок-гейт статистики (§7 spec 2026-07-15 + задача 2, дефект 2). Демо-данные
    # показываются ТОЛЬКО если явно включены (`mock_stats_enabled`) И одновременно
    # нет реальных метрик, не истёк срок и клиентов меньше порога. Без явного флага
    # гейт закрыт всегда — демо не должны всплывать сами по себе (например, на
    # приёмке у заказчика). По умолчанию флаг выключен.
    mock_stats_enabled: bool = False  # env: MOCK_STATS_ENABLED
