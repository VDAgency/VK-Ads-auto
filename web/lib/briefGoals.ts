 ...
 * списков-исключений по имени конкретной цели — так «Вовлечение в готовый
 * объект» (клип заблокирован, но четыре других варианта доступны) остаётся
 * открытой, а «Заявка через Senler» (единственная площадка которой прошла
 * боевой прогон 2026-08-24, `web/lib/briefSurfaces.ts`, `enabled: true`)
 * открылась сама следующей сборкой — без единой строчки кода, завязанной на
 * конкретное название цели. Ровно так же откроется любая будущая цель, как
 * только её последняя площадка пройдёт боевую проверку.
 */
...
export const BRIEF_GOAL_TABS: BriefGoalTab[] = [
  ...
  },
  {
    ...
    // элемент (integrations/vk_surfaces.VK_SENLER), и он открыт с 2026-08-24 —
    // вкладка доступна, без единичного исключения в коде.
    key: "senler",
