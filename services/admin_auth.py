...
Честно: досрочного отзыва отдельной сессии здесь нет — токен просто подписан HMAC и
действует до истечения TTL. Погасить разом ВСЕ выданные admin-сессии можно только
сменой `SECRET_KEY` (тогда и все magic-link, и все сессии перестанут проходить проверку
подписи) — прицельного механизма («выйти на всех устройствах», «отозвать сессию N») нет.
"""
