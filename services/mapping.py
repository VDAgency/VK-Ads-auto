...
Поддержаны четыре цели — подписчики, заявки через лид-форму, сообщения и заявка
через Senler (`services.brief_parser.Goal`).
"""
...
class UnsupportedBriefGoalError(Exception):
    ...
    Сейчас все четыре цели перечисления `services.brief_parser.Goal` (подписчики,
    лид-форма, сообщения, Senler) поддержаны — эта ошибка на них больше не
    срабатывает. Класс остаётся ради будущих целей, которые когда-нибудь попадут
    в `Goal`, но раскладку под них ещё не напишут: тогда бриф с такой целью снова
    будет отклонён здесь, а не голым `ValueError`. `services.launch_service.
    launch_from_creative` ловит эту ошибку и транслирует в свой `UnsupportedGoalError`,
    чтобы роутеры и бот отвечали честным 422, а не 500 с трассировкой (не путать с
    `UnsupportedGoalError`: тот — про параметр `goal`, который оператор передаёт
    явно при запуске).
    """
