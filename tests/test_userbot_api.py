...
def test_health_endpoint_authorized(tmp_path: Path) -> None:
    ...
    assert resp.json() == {"authorized": True, "phone": "+79990001122"}


def test_health_endpoint_unauthorized(tmp_path: Path) -> None:
...
def test_send_endpoint_ok(tmp_path: Path) -> None:
    ...
    client, _ = make_client(fake, tmp_path=str(tmp_path), saved_session=True)
    with _app_with(client) as tc:
        resp = tc.post("/send", json={"username": "@ivanov", "text": "бриф"})
    assert resp.status_code == 200
...
def test_send_endpoint_username_not_occupied(tmp_path: Path) -> None:
    ...
    client, _ = make_client(fake, tmp_path=str(tmp_path), saved_session=True)
    with _app_with(client) as tc:
        resp = tc.post("/send", json={"username": "@nobody", "text": "t"})
...
def test_send_endpoint_peer_flood_429(tmp_path: Path) -> None:
    ...
    client, _ = make_client(fake, tmp_path=str(tmp_path), saved_session=True)
    with _app_with(client) as tc:
        resp = tc.post("/send", json={"username": "@x", "text": "t"})
    assert resp.status_code == 429
    assert resp.json()["error"] == "peer_flood"


def test_send_endpoint_session_expired_401(tmp_path: Path) -> None:
    client, _ = make_client(tmp_path=str(tmp_path), saved_session=False)
    with _app_with(client) as tc:
        resp = tc.post("/send", json={"username": "@x", "text": "t"})
...
def test_auth_flow_endpoints_without_2fa(tmp_path: Path) -> None:
    ...
    with _app_with(client) as tc:
        start = tc.post("/auth/start", json={"phone": "+79990001122"})
        assert start.status_code == 200
        hash_ = start.json()["phone_code_hash"]

        code = tc.post(
            "/auth/code",
            json={"phone": "+79990001122", "code": "12345", "phone_code_hash": hash_},
        )
...
def test_auth_code_endpoint_with_2fa_then_password(tmp_path: Path) -> None:
    ...
    with _app_with(client) as tc:
        ...
        pwd = tc.post("/auth/password", json={"password": "cloud-pass"})
...
def test_auth_code_invalid_returns_400(tmp_path: Path) -> None:
    ...
    client, _ = make_client(fake, tmp_path=str(tmp_path))
    with _app_with(client) as tc:
        tc.post("/auth/start", json={"phone": "+79990001122"})
        resp = tc.post(
            "/auth/code",
            json={"phone": "+79990001122", "code": "0", "phone_code_hash": "h"},
        )
    assert resp.status_code == 400
