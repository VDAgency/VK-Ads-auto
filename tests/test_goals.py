...
`NO_CREATIVE_GOAL` — код цели для запуска без креатива (объявлением служит сам
пост/клип/трек — см. `services/goals.py::goal_for_target_type`).
"""

from __future__ import annotations

from services.brief_parser import Goal
from services.goals import NO_CREATIVE_GOAL, launch_goal_title, launch_goals
...
def test_no_creative_goal_is_subscribers() -> None:
    """Продвижение готового поста/клипа/трека всегда идёт под целью «подписчики»."""
    assert Goal.SUBSCRIBERS.value == NO_CREATIVE_GOAL
