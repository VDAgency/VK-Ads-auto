...
успех, CLAUDE.md §7); `"nothing_to_update"` — синкать было нечего (кампания ещё
не запущена, уже остановлена оператором или упала при запуске) — это НЕ сбой,
пометка нейтральная, без ⚠️; `"updated"` — молча, пометки нет.
"""
...
# «Нечего обновлять» (A3) — кампания вне `launched`/`moderation`
# (`prepared`/`stopped`/`failed`), синк площадку не спрашивал. Формулировка обязана
# быть верной для ВСЕХ трёх случаев разом — в том числе для `stopped` (кампанию
# оператор сам остановил ПОСЛЕ того, как она откручивалась и набрала статистику,
# см. `services.launch_service.stop_campaign`): "не запущена" была бы прямой ложью
# именно для неё и противоречила бы честному "остановлен" из `_STATUS_HINT` в
# списке кабинетов. Это НЕ сбой — пометка нейтральная, без ⚠️, иначе оператор
# увидит тревогу под каждым неподнятым кабинетом (баг, из-за которого A2 перелечило
# исходную честную ошибку).
_SYNC_NOTHING_NOTE = "\n\nНовых данных пока нет — показаны последние сохранённые цифры."
