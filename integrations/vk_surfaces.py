...
# сообщение"». Боевой зонд 2026-08-23 (`POST /ad_plans.json`, package_id=3127, баннер
# без содержимого) ПОДТВЕРДИЛ пакет и objective: VK не пожаловался на `objective`, а
# перешёл сразу к проверке шаблонов — значит `socialengagement` для 3127 верен.
# Ответ 400 назвал полный список разрешённых шаблонов пакета — 13 id:
#   [486, 422, 525, 527, 400, 401, 530, 338, 529, 339, 145, 150, 537]
# У пакета сообщества 3122 (`VK_COMMUNITY`) тот же зонд вернул ТОТ ЖЕ набор id
# (отличается только порядок 145/150) — площадка 3127 структурно идентична 3122,
# разница только в пакете и кнопке.
# Раскрыты через `/banner_patterns.json` (в этой сессии — не запрашивалось, только
# `/ad_plans.json`) только 10 из 13: те же id, что уже были в таблице ниже под старую
# догадку «шаблоны как у VK_COMMUNITY» — теперь они не догадка, а подтверждённое
# подмножество реального списка пакета. Слот кнопки `cta_community_vk` подтверждён
# отдельно — запросом к `/banner_patterns.json` по шаблону 529.
# ⚠️ Три id — 486, 422, 537 — зонд подтвердил как ДОПУСТИМЫЕ пакетом 3127, но их
# медиа-слот (форма/соотношение сторон) в этой сессии не раскрывался: нужен ещё один
# запрос к `/banner_patterns.json` по каждому id. Это не блокирует площадку — 10
# подтверждённых шаблонов ниже сами по себе валидны для 3127, просто клиенту пока не
# предлагаются оставшиеся 3 формы креатива (какая-то доп. пропорция/длительность).
...
# `verified=True`: отдельный боевой прогон именно под именем Senler проведён
# руководителем 2026-08-24 на сообществе DJ BEAUTY (228817082) — токен привязан
# (`connected: true`), бриф с площадкой Senler принят, кампания создана и прочитана
# напрямую из VK: `{"id": 28802233, "name": "Senler · ПРОГОН Senler", "status":
# "blocked", "objective": "socialengagement", "package_id": 3127}` — верный пакет,
# верная цель, верный префикс имени. Тестовые кампании удалены, прод очищен.
VK_SENLER = Surface(
    kind="senler",
